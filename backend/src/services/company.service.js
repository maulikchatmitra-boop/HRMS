...
/**
 * Gets company details by companyId.
 ...
 */
export const updateCompany = async (companyId, updateData, actorId) => {
  const oldCompany = await Company.findById(companyId);
  if (!oldCompany) {
    throw new Error('Company not found.');
  }

  const allowedUpdates = ['companyName', 'email', 'phone'];
  const actualUpdates = {};
  ...
  await logAction({
    ...
    newData: updatedCompany.toObject()
  });
